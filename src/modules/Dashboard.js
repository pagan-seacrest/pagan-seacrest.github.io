...
export default class Dashboard {
  constructor () {
    ...
    root.addEventListener("click", (ev) => {
        if (document.getElementById("edit-form") === null) {
          ...
          ? document.getElementById("edit-form").remove() : false;  
        }
    });
  }
  ...
  filter () {
    const modal = new Modal({title: "Фільтрувати візити за", id: "filter", place: root});
    const filter = modal.add();
    new Select({id: "filter-doctor-select", place: filter}).filterBy(config.filterValues.doctor, filter)
      .addEventListener("click", (ev) => {
          ev.target.value === "Всі" ? config.searchValues.doctor.all = true :
          config.searchValues.doctor.all = false;
          config.searchValues.doctor.picked = ev.target.value;
      });
    new Select({id: "filter-urgency-select", place: filter}).filterBy(config.filterValues.urgency, filter)
      .addEventListener("click", (ev) => {
        ev.target.value === "Всі" ? config.searchValues.urgency.all = true :
        config.searchValues.urgency.all = false;
        config.searchValues.urgency.picked = ev.target.value;
      });
    new Input({place: filter, type: "text", id: "filter-purpose"}).filter(config.filterValues)
      .addEventListener("blur", (ev) => {
        config.searchValues.purpose.value = ev.target.value;
        ev.target.value === undefined || ev.target.value === "" ?
          config.searchValues.purpose.content = false : config.searchValues.purpose.content = true;
      });
  ...
  search (cards, dashboard) {
    cards.map(card => {
      if (card.children[0].textContent !== config.searchValues.doctor.picked) {
        return card;
      }
    }).map(card => {
      if (card === undefined) {
        ...
        if (card.children[1].children[2].lastElementChild.textContent !== config.searchValues.urgency.picked) {
          return card;
        }
      }
    }).map(card => {
      if (card === undefined) {
        ...
        if (card.children[1].children[1].lastElementChild.textContent !== config.searchValues.purpose.value) {
          return card;
        }
      }
    }).forEach(card => {
  ...
  card (place, visit) {
    ...
    new Button({type: "button", value: "Розгорнути", id: `folding-${visit.id}`, className: "folding", place: wrapper});
  ...
  reload () {
   ...
   escape (event) {
    if (document.getElementById("visit-form") === null) {
      ...
      ? document.getElementById("visit-form").remove() : false;
    }
  }
